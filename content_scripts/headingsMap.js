(function () {
  ...
  function querySelectorAll(selector, root = document) {
    ...
    if (root instanceof Document) {
      startNode = root.documentElement;
    }
    (function walk(node) {
  ...
  function HTML5Outline(documentWindow, documentIndex) {
    var outLine = null,
      ...
      generateHTMLListFromSections = function (sections) {
        ...
        for (
          var i = 0, sectionsLength = sections.length;
          i < sectionsLength;
          i++
        ) {
      ...
      getHeaderElementContent = function (element) {
        if (isHeaderElement(element)) {
          return (
            getText(element) ||
            element.innerText ||
            noHeadClass + "no content inside " + element.nodeName
          );
        }
        return "" + element;
      },
      ...
      xxxxxx = function (a) {
        ...
        if (isSectioningElement(a) || isSectioningRoot(a)) {
          ...
          if (dddddd.heading) {
            if (
              getHeaderElementLevel(a) >=
              headingLevel(getLastSection(eeeeeee.outline))
            ) {
      ...
      yyyyyyy = function (a) {
        ...
        if (isHeaderElement(b)) {
          ...
          if (
            (isSectioningElement(a) || isSectioningRoot(a)) &&
            !dddddd.heading
          ) {
      ...
      isHeadingRelatedElement = function (pattern) {
        return function (nodeElement) {
          return (
            nodeElement &&
            nodeElement.tagName &&
            new RegExp(pattern, "i").test(nodeElement.tagName.toUpperCase())
          );
        };
      },
      ...
      isSectioningRoot = isHeadingRelatedElement(
        "^BLOCKQUOTE|BODY|DETAILS|FIELDSET|FIGURE|TD$"
      ),
      isSectioningElement = isHeadingRelatedElement(
    ...
    outlineFactory.prototype = {
      ...
      asHTML: function () {
        ...
        return [
          getTreeRow(getHeaderElementContent(this.heading), self),
          generateHTMLListFromSections(self.sections),
        ];
        ...
        function getTreeRow(headingContent, self) {
          ...
          if (headingContent.indexOf(noHeadClass) === -1) {
            ...
            itemTextContent =
              " Untitled (" + itemTextContent.replace(noHeadClass, "") + ")";
    ...
    function addLevelIndex(list, type) {
      ...
      for (
        var i = 0, childNodesListLength = childNodesList.length;
        i < childNodesListLength;
        i++
      ) {
        ...
        childNode.childNodes[0].insertBefore(
          span,
  ...
  function headingsMap(documentWindow, documentIndex) {
    var mainList,
      ...
      parentList,
      linkElement,
      descendantListElement,
      headerTextNode,
      precedingHeadLevel,
      parentListClass,
      classValue,
      headLevelsSpanElement,
      headLevelsSpanTextNode,
      previous = 0,
      previousCorrect = 0,
      currentLevel = 0,
      headingsMapSection = generateSectionForMap(documentWindow, documentIndex),
      documentToCheck = documentWindow.document,
      headingElements = querySelectorAll(
        "h1, h2, h3, h4, h5, h6",
        documentToCheck
      );
    ...
    for (
      var i = 0, headingElementsLength = headingElements.length;
      i < headingElementsLength;
      i++
    ) {
      ...
      if (currentLevel > previous) {
        ...
        while (parentList.parentNode.getAttribute("class") != "hroot") {
          ...
          if (parentListClass.indexOf("h") >= 0) {
            ...
            precedingHeadLevel = parseInt(
              parentListClass.replace(classPrefix, "")
            );
      ...
      if (currentLevel > previous + 1) {
        if (
          (i === 0 && showHeadErrorH1 === true && showHeadError === true) ||
          (i > 0 && showHeadError === true)
        ) {
  ...
  function getWidget() {
    ...
    widgetLists = querySelectorAll("ul ul", widgetContent);

    for (
      var i = 0, widgetListsLenght = widgetLists.length;
      i < widgetListsLenght;
      i++
    ) {
    ...
    function getWidgetContent(headingMap, outlineMap) {
      ...
      headingsTabLink = generateTabLink(
        "Headings Structure",
        headingsTabLinkId,
        headingsMapId
      );
      ...
      outlineMapTabLink = generateTabLink(
        "HTML5 Outline",
        outlineTabLinkId,
        outlineMapId
      );
  ...
  function getText(element) {
    ...
    for (
      var i = 0, childNodesLength = childNodes.length;
      i < childNodesLength;
      i++
    ) {
    ...
    function isElementWithAltText(element) {
      ...
      return (
        (tagName === "img" &&
          element.getAttribute("src").indexOf("moz-extension://") != 0) ||
        tagName === "area" ||
        (tagName === "input" &&
  ...
  function openWidget(widgetContent) {
    ...
    onEndingDOMChangeCallback = function (mutations) {
      var headingsMapWidget = getWidget(),
        iframeBodyInnerHTML = iframeBody
          ? replaceAll(iframeBody.innerHTML, ' style="display: none;"', "")
          : "",
        headingsMapWidgetInnerHTML =
          '<div id="headingsMapWrapper">' +
          replaceAll(
            headingsMapWidget.innerHTML,
            ' style="display: none;"',
            ""
          ) +
          "</div>";
      ...
      if (
        replaceAll(iframeBodyInnerHTML, " collapsed", "") !=
        replaceAll(headingsMapWidgetInnerHTML, " collapsed", "")
      ) {
        updateWidget(headingsMapWidget);
      }
    };
    bodyMutationEndingObserver = addMutationObserver(
      body,
      mutationObserverParams,
      debounceFn(onEndingDOMChangeCallback, false)
    );
  }
  ...
  function createIframeWidget(widgetContent) {
    var xmlhttp = new XMLHttpRequest(),
      baseURL = chrome.runtime.getURL("html/"),
      iframeCSS,
      iframeHead,
      iframeWidget = createElement("iframe", {
        id: headingsMapIframeWrapperId,
      }),
    ...
    xmlhttp.onload = function (e) {
      if (xmlhttp.readyState === 4) {
        if (xmlhttp.status === 200) {
          ...
          iframeHead =
            '<base href="' + baseURL + '" /><style>' + iframeCSS + "</style>";
  ...
  function cleanHeaderIds() {
    var headerId,
      headerIdParts,
      section,
      headingElements = querySelectorAll("h1, h2, h3, h4, h5, h6", document);
    ...
    for (
      ...
      if (
        headerIdParts.length > 1 &&
        headerIdParts[0] === "" &&
        Number.isInteger(parseInt(headerIdParts[1]))
      ) {
    ...
    for (
      var i = 0, sectionsIdLength = sectionsId.length;
      i < sectionsIdLength;
      i++
    ) {
  ...
  function getDocuments(act, previous) {
    ...
    for (var i = 0, framesLength = frames.length; i < framesLength; i++) {
      ...
      if (
        currentDocument.getElementsByTagName("frame").length ||
        currentDocument.getElementsByTagName("iframe").length
      ) {
  ...
  function generateSectionForMap(documentWindow, documentIndex) {
    var sectionSubHeaderContent,
      ...
      titleElements = querySelectorAll("title", documentToCheck),
      titleElement = titleElements.length
        ? querySelectorAll("title", documentToCheck)[0]
        : false,
